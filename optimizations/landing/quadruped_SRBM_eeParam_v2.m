...
nodes_force_idx = cell(model.NLEGS, 1);
nodes_posn = cell(model.NLEGS, 1);
nodes_posn_idx = cell(model.NLEGS, 1);
for leg = 1:model.NLEGS
    nodes_force_idx{leg} = opti.parameter(1, numel(contactState_forces{leg}));
    nodes_posn_idx{leg} = opti.parameter(1, numel(contactState_posns{leg}));
    for i = 1:numel(contactState_forces{leg})
        nodes_force{leg}{i}.coef = opti.variable(3, 4);
        nodes_force{leg}{i}.coef_dot = getDerivCoef(nodes_force{leg}{i}.coef);
        nodes_force{leg}{i}.duration = contactDuration_forces{leg}(i);
        if contactState_forces{leg}(i) == 1
            nodes_force{leg}{i}.cs = 1;
        else
            nodes_force{leg}{i}.cs = 0;
        end
        nodes_force{leg}{i}.idx = i;
        time_force{leg}(1, i + 1) = sum(contactDuration_forces{leg}(1:i));
    end
    for j = 1:numel(contactState_posns{leg})
        nodes_posn{leg}{j}.coef = opti.variable(3,4);
        nodes_posn{leg}{j}.coef_dot = getDerivCoef(nodes_posn{leg}{j}.coef);
        nodes_posn{leg}{j}.duration = contactDuration_posns{leg}(j);
        if contactState_forces{leg}(j) == 1
            nodes_posn{leg}{j}.cs = 1;
        else
            nodes_posn{leg}{j}.cs = 0;
        end
        nodes_posn{leg}{j}.idx = j;
...
for leg = 1:model.NLEGS
    for i = 1:numel(nodes_force{leg})
        f_0 = nodes_force{leg}{i}.coef(:, 1); % x0 of xyz force spline
        f_1 = nodes_force{leg}{i}.coef(:, 3); % x1 of xyz force spline
        if (nodes_force{leg}{i}.cs == 0)
            % alternatively, could constrain f_i == 0? unsure which is more efficient
            opti.subject_to(nodes_force{leg}{i}.coef(:) == zeros(12, 1));
        else
            opti.subject_to(f_0(3) >= zeros(3, 1));     % ground normals >= 0 in contact
            % opti.subject_to(f_1(3) >= zeros(3, 1));   % needed? with continuity, seems unnecessary...
    ...
    for j = 1:numel(nodes_posn{leg})
        if (nodes_posn{leg}{j}.cs == 1)
            opti.subject_to(nodes_posn{leg}{j}.coef(1,2:4) == zeros(1, 3));
            opti.subject_to(nodes_posn{leg}{j}.coef(2,2:4) == zeros(1, 3));
            opti.subject_to(nodes_posn{leg}{j}.coef(3,:) == zeros(1, 4));
        end        
    end
    opti.subject_to(sum(phaseDurations{leg}) == T);               % phase durations of legs sum to total time
    for k = 1:numel(contactSequence{leg})
        opti.subject_to(0 <= phaseDurations{leg}(k) <= T);        % phase durations must be positive and bounded
    end
end
...
for leg = 1:model.NLEGS
    for i = 1:numel(nodes_force{leg})-1
        for xyz = 1:3
            coef_prev = convertHermiteCoef(nodes_force{leg}{i}.coef(xyz, :), nodes_force{leg}{i}.duration);
            coef_next = convertHermiteCoef(nodes_force{leg}{i+1}.coef(xyz, :), nodes_force{leg}{i+1}.duration);
            
            prevSpline_end_f = polyval(coef_prev', nodes_force{leg}{i}.duration);
            prevSpline_end_fDot = polyval(getDerivCoef(coef_prev)', nodes_force{leg}{i}.duration);
            nextSpline_end_f = polyval(coef_next', 0);
            nextSpline_end_fDot = polyval(getDerivCoef(coef_next)', 0);
            
            opti.subject_to(prevSpline_end_f == nextSpline_end_f);
            opti.subject_to(prevSpline_end_fDot == nextSpline_end_fDot);
        end
    end
    for j = 1:numel(nodes_posn{leg})-1
        for xyz = 1:3
            coef_prev = convertHermiteCoef(nodes_posn{leg}{j}.coef(xyz, :), nodes_posn{leg}{j}.duration);
            coef_next = convertHermiteCoef(nodes_posn{leg}{j+1}.coef(xyz, :), nodes_posn{leg}{j+1}.duration);
            
            prevSpline_end_p = polyval(coef_prev', nodes_posn{leg}{j}.duration);
            prevSpline_end_pDot = polyval(getDerivCoef(coef_prev)', nodes_posn{leg}{j}.duration);
            nextSpline_end_p = polyval(coef_next', 0);
            nextSpline_end_pDot = polyval(getDerivCoef(coef_next)', 0);
            
            opti.subject_to(prevSpline_end_p == nextSpline_end_p);
            opti.subject_to(prevSpline_end_pDot == nextSpline_end_pDot);
        end
    end
end
...
for k = 1:N_timesteps-1
    ...
    f_idx = full(getSplineIndex(time_force{1}, t_global_test));
    p_idx = full(getSplineIndex(time_posn{1}, t_global_test));
...
%         p_rel = R_world_to_body*(p_k(1:3) - r_hip);
%         kin_box_x = 0.05;
%         kin_box_y = 0.05;
%         kin_box_z = 0.30;
%         
...
%         opti.subject_to(-kin_box_x <= p_rel(1) + 0.05 <= 0)
%         
%     end
...
% out IPOPT
for i = 1:model.NLEGS
    N_phases = length(phaseDurations{i});
    opti.set_initial(phaseDurations{i}, (T_val/N_phases)*ones(1, N_phases));
end
...
for leg = 1:model.NLEGS
    for xyz = 1:3
        for i = 1:numel(nodes_force{leg})
            dur = sol.value(nodes_force{leg}{i}.duration);
            f_star{xyz, leg}(i, :) = convertHermiteCoef(sol.value(nodes_force{leg}{i}.coef(xyz, :)), dur);
        end
        for j = 1:numel(nodes_posn{leg})
            dur = sol.value(nodes_posn{leg}{j}.duration);
            p_star{xyz, leg}(j, :) = convertHermiteCoef(sol.value(nodes_posn{leg}{j}.coef(xyz, :)), dur);
        end
    end
end
