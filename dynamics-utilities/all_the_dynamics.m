function [ H,C,p0,J0,Jdqd,pf0,Jf0,Jdfqd0,vvf,ph0,Jh0,vvh ] = all_the_dynamics( model, q, qd, f, is_symbolic )
...
for i = 1:model.NLEGS      
    ...
    if (model.NARMS == 0) % quad only uses linear velocity / force component of Jacobian
        Jf0{i} = [zeros(3,3) eye(3)] * Xr0{j} * model.Xfoot{i} * Xr0{j}' * J0{j}; % transform world jacobian to foot
    else % Humanoid uses full jacobian
        Jf0{i} = Xr0{j} * model.Xfoot{i} * Xr0{j}' * J0{j}; % transform world jacobian to foot
    end
...
ph0 = repmat({zeros(3,1)},model.NLEGS,1);
vvh = repmat({zeros(6,1)},model.NLEGS,1);
Jh0 = repmat({zeros(3,3)},model.NLEGS,1);
for i = 1:model.NARMS
    %disp(i);% loop through hands
    j = model.b_hand(i);                           % body containing hand
    [~,ph0{i}] = plux_2(model.Xhand{i} * X0{j});   % origin to hand translation, world coordinates
    vvh{i} = Xr0{j} * model.Xhand{i} * v{j};          % hand velocity
    
    % Only need linear velocity component for the hands
    Jh0{i} = [zeros(3,3) eye(3)] * Xr0{j} * model.Xhand{i} * Xr0{j}' * J0{j}; % transform world jacobian to hand
